# Program output: Thesecuetmessage-isWh-nusing-streamcipher
#                                         never-usethekeymorethanonce
# Secret message: The secret message is when using a stream cipher, never use the key more than once.
...
def main():
    decoded_ciphertexts = [ct.decode('hex') for ct in ciphertexts]
    ...
    for ciphertext in decoded_ciphertexts:
        ...
        cts_xored_w_each_other = [strxor(ciphertext, diff_ciphertext) for diff_ciphertext in decoded_ciphertexts if diff_ciphertext != ciphertext]

        for idx in range(ciphertext_length):
            space_counter = 0
            for xor in cts_xored_w_each_other:
                # Note that xor[idx] == '\x00' evaluates to True when the character of ciphertext x and y matched at that index.
                # Let us assume that if xor[idx] == '\x00' evaluates to true, both ciphertexts have a space there.
                # Therefore, if xor[idx] != '\x00' evaluates to true and xor[idx] is not a letter in the alphabet, then there is no space marker there 
                # in our ciphertext. Otherwise, there is.
                # Let's keep track of the number of spaces we think we have in our ciphertext.
                if idx < len(xor) and xor[idx] != '\x00' and xor[idx] not in string.letters:
                    continue
                else:
                    space_counter += 1
            # If we see that xor[idx] == '\x00' evaluated to true in at least 7 different pairings of the ciphertexts, then we should be pretty confident that 
            # there is a space at this index in this particular ciphertext.
            if space_counter > 7:
                spaces_guess[idx] = " "
    ...
    for idx in range(n_ciphertexts):
        ...
        for jdx in range(ct_length):
            if jdx < len(msg_guess) and ct_spaces[jdx] == " ":
                # If you know that the ciphertext had a space, then you know that the XOR of the ciphertext's and the target's character would result in 
                # a copy of the target's character, with the opposite case.
